...
1. **Windows**:
    ...
    - Ensure the `opuslib` package is correctly installed.

2. **Linux/macOS**:
    - Install `libopus` using:
       ```sh
...
self.websocket = await websockets.connect(
      self.WEBSOCKET_URL, 
      extra_headers=headers  # In newer versions, change to additional_headers=headers
)
```

### **Solution**

- **Newer Versions**: Replace `extra_headers` with `additional_headers`.
- **Older Versions**: Replace `additional_headers` with `extra_headers`.
...
## 5. `ModuleNotFoundError: No module named '_tkinter'` (Common on macOS M1/M2)
...
```
Found opus library: D:\xiaozhi\PC\py-xiaozhi-main\libs\windows\opus.dll
...
Ensure the opus dynamic library is correctly installed or in the correct location.
```
...
1. **Python Version Compatibility**:
    - This error is often related to Python version issues, especially with Python 3.13.
    - Use Python versions 3.9–3.12.
...
3. **Place `opus.dll` in Correct Locations**:
    ```sh
    # Ensure opus.dll is copied to these locations:
...
## 7. `error: subprocess-exited-with-error` (Failed to Install `numpy`)
...
1. **Ensure `numpy` Version Compatibility**:
    - `numpy==2.0.2` may have build issues. Try installing a stable version:
       ```sh
       pip install numpy==1.24.3
       ```

    - If no specific version is required, install the latest stable version:
       ```sh
       pip install numpy
       ```
